...
export function calculateNavyBodyFat(
  ...
  let bf = 0;
  if (gender === 'male') {
    const diff = Math.max(0.5, waistCm - neckCm);
    const validHeight = Math.max(50, heightCm);
    bf = 495 / (1.0324 - 0.19077 * Math.log10(diff) + 0.15456 * Math.log10(validHeight)) - 450;
  } else {
    const sum = Math.max(0.5, waistCm + hipCm - neckCm);
    const validHeight = Math.max(50, heightCm);
    bf = 495 / (1.29579 - 0.35004 * Math.log10(sum) + 0.221 * Math.log10(validHeight)) - 450;
  }

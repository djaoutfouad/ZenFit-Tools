...
export const CalculatorWorkspace: React.FC<CalculatorWorkspaceProps> = ({
  ...
  /**
   * Collects the current calculator's input and output state and generates
   * a downloadable .txt file containing a formatted summary of the calculation,
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
          ...
          <div className="flex items-center gap-2">
            {calc.readTime && (
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium bg-slate-100 text-slate-700 border border-slate-200 shadow-xs">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-8">
          ...
          <div className="lg:col-span-7 bg-white rounded-3xl p-6 sm:p-7 border border-slate-200 shadow-sm flex flex-col justify-between">
            ...
            {/* Fitness Log Export & Integration Section */}
            <div className="mt-8 pt-5 border-t border-slate-100">
              <div className="p-4 sm:p-5 rounded-2xl bg-slate-50/90 border border-slate-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div>
                ...
                <div className="flex items-center gap-2 shrink-0 w-full sm:w-auto">
                  <button
                    id="export-card-results-btn"
                    onClick={handleExportResults}
                    className="flex-1 sm:flex-none inline-flex items-center justify-center gap-1.5 px-4 py-2 bg-slate-900 hover:bg-slate-800 active:scale-95 text-white rounded-xl text-xs font-bold transition-all shadow-xs"
                    title="Export calculation results as a downloadable .txt file"
                  >
                    {exportSuccess ? <Check className="w-3.5 h-3.5 text-amber-400" /> : <Download className="w-3.5 h-3.5 text-amber-400" />}
                    <span>{exportSuccess ? 'Downloaded .TXT!' : 'Export Results'}</span>
                  </button>
                  <button
                    id="export-card-copy-btn"
                    onClick={handleCopySummary}
                    className="flex-1 sm:flex-none inline-flex items-center justify-center gap-1.5 px-3 py-2 bg-white border border-slate-200 hover:bg-slate-100 text-slate-800 rounded-xl text-xs font-bold transition-all shadow-2xs"
                    title="Copy Formatted Summary"
                  >
                    {copied ? <Check className="w-3.5 h-3.5 text-emerald-600" /> : <FileText className="w-3.5 h-3.5 text-amber-600" />}
                    <span>{copied ? 'Copied!' : 'Copy Text'}</span>
                  </button>
                  <button
                    id="export-card-csv-btn"
